...
st.write(
    """
    In this example, we have a thread which obtains a `SHARED` lock on the database, but doesn't commit the transaction until 10s later.
...
with st.echo():
    ...
    def get_shared_lock(conn: sqlite3.Connection, thread_num: int):
        ...
        # The connection is not committed until 10s later
...
st.page_link("pages/5_Recursive_Cursors.py", label="Next: Recursive Cursors", icon=":material/arrow_forward:")